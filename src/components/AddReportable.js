...
export class AddReportableInReport extends UI {
  ...
  addReportable () {
    ...
    if (this.state.text.trim() !== '') {
      reconciler.put(Mutations.UPDATE_STATUS, taskId, [snapshotTypes[type], {
        'reportable/title': this.state.text,
        'reportable/reporter': taskOwner,
        'reportable/order': newItemPosition
      }])
    }
  }
        ...
        <View style={addReportableStyle.overlay}>
          <View style={{flexDirection: 'row', paddingHorizontal: 36}}>
            <View style={[addReportableStyle.bubble, {flex: 1, flexDirection: 'column', marginBottom: 18}]}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center'}}>
                <TouchableOpacity
		  style={[ addReportableStyle.OKButton, {backgroundColor: ReportableColors[type]} ]}
                  onPress={e => { this.addReportable() }}>
                  <Text style={addReportableStyle.buttonLabel}>Hinzufügen</Text>
                </TouchableOpacity>
              </View>
              <TouchableOpacity
		style={addReportableStyle.cancelButton}
